...
let currentEnchantType = 'All'; 
let showRarityColors = true;    
...
function cycleFilterDisplay() {
    ...
    updateComparatorLore();

    applyCurrentFilter();
    renderInventory(currentPage); 
    updateDebugInfo(`🔍 Filter: ${currentFilterDisplay}`);
}
function applyCurrentFilter() {
    const enchantmentSlots = [12,13,14,15,16,21,22,23,24,25,30,31,32,33,34];

if (currentFilterDisplay === 'All' && currentEnchantType === 'All') {
...
if (matchesFilter(originalItem, currentFilterDisplay) && matchesEnchantType(originalItem, currentEnchantType)) {
...
function matchesEnchantType(item, enchantType) {
    if (enchantType === 'All') return true;
    
    const loreText = Array.isArray(item.lore) ? item.lore.join(' ') : '';
    
    if (enchantType === 'Refined') {
        return loreText.includes('Refined');
    }
    
    if (enchantType === 'Arcane') {
        return loreText.includes('Arcane');
    }
    
    return true;
}

function cycleEnchantType() {
    const typeCycle = ['All', 'Refined', 'Arcane'];
    const currentIndex = typeCycle.indexOf(currentEnchantType);
    const nextIndex = (currentIndex + 1) % typeCycle.length;
    
    currentEnchantType = typeCycle[nextIndex];
    updateComparatorLore();
    
    applyCurrentFilter();
    renderInventory(currentPage);
    updateDebugInfo(`✨ Enchant type: ${currentEnchantType}`);
}

function updateComparatorLore() {
    const filterCycle = ['All', 'Armor', 'Tool', 'Weapon', 'Ranged'];
    const typeCycle = ['All', 'Refined', 'Arcane'];
    
    const nextFilterIndex = (filterCycle.indexOf(currentFilterDisplay) + 1) % filterCycle.length;
    const nextFilter = filterCycle[nextFilterIndex];
    
    const nextTypeIndex = (typeCycle.indexOf(currentEnchantType) + 1) % typeCycle.length;
    const nextType = typeCycle[nextTypeIndex];
    const typeColor = nextType === 'Arcane' ? '&6' : 
                     nextType === 'Refined' ? '&7' : '&f';
    
    pages.forEach(page => {
        const filterItem = page.items.find(it => it.slot === 5 && it.id === 'minecraft:comparator');
        if (filterItem) {
            filterItem.name_raw = `&fCurrently viewing: &f${currentFilterDisplay} ${currentEnchantType !== 'All' ? currentEnchantType : ''} enchants`.trim();
            filterItem.lore = [
                `&e► &lLeft &r&eClick to view &f${nextFilter}&e enchants`,
                `&e► &lRight &r&eClick to view ${typeColor}${nextType}&e enchants`
            ];
        }
    });
}
...
function getFilteredIcon(item, filter) {
    ...
    if (combinedText.includes('sprees')) {
        return 'minecraft:diamond_pickaxe';
    }
    if (combinedText.includes('mystical touch')) {
        return 'minecraft:diamond_pickaxe';
    }
    if (combinedText.includes('moltens touch') || combinedText.includes('molten touch')) {
        return 'minecraft:diamond_pickaxe';
    }
    const weaponEnchants = ['uppercut', 'militia', 'heart stealer', 'bloodlust', 'bash', 'stormhailer', 'crimson', 'disarmor', 'meltdown'];
...
function sortEnchantments() {
    ...
    Object.keys(originalEnchantmentOrder).forEach(pageNum => {
        ...
        Object.keys(pageItems).forEach(slotIndex => {
            ...
            if (item && item.id && item.name_raw && 
                matchesFilter(item, currentFilterDisplay) && 
                matchesEnchantType(item, currentEnchantType)) {
    ...
    renderInventory(currentPage);
    updateDebugInfo(`✅ ${currentFilterDisplay} ${currentEnchantType !== 'All' ? currentEnchantType : ''} enchantments sorted alphabetically`.trim());
}
function updateSortComparator(isSorted) {
    pages.forEach(page => {
        const sortItem = page.items.find(it =>
            it.slot === 50 && it.id === 'minecraft:comparator' &&
            it.name_raw.includes('Sort Enchantments')
        );
        if (sortItem) {
            sortItem.lore = isSorted
                ? ["Currently Viewing: Alphabetical Order", "&e&lClick to sort by &f&lDefault &e&lOrder"]
                : ["Currently Viewing: Default Order", "&e&lClick to sort by &f&lAlphabetical &e&lOrder"];
        }
    });
    renderInventory(currentPage);
}
...
function loadRandomEnchantsIntoCe2() {
    ...
    const revealSlots = [12, 13, 14, 21, 22, 23, 30, 31, 32];
...
function showTooltip(e, item){
    ...
    if(item.lore && item.lore.length > 0){
        for(let i = 0; i < item.lore.length; i++){
            let loreLine = item.lore[i];
            if(i === 0 && !showRarityColors && typeof loreLine === 'string') {
                loreLine = loreLine.replace(/^(&[0-9a-f])(Common|Uncommon|Rare|Epic|Mythic|Legendary)/, '&f$2');
            }
    ...
    for(let i = 0; i < 54; i++){
        ...
        if(item && item.id){
            ...
            slot.addEventListener("contextmenu", (event) => {
                if (i === 5 && item.id === 'minecraft:comparator') {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const rarityToggle = document.getElementById('rarity-colors');
    if (rarityToggle) {
        rarityToggle.checked = showRarityColors;
        rarityToggle.addEventListener('change', (e) => {
            showRarityColors = e.target.checked;
            renderInventory(currentPage);
            updateDebugInfo(`Rarity colors: ${showRarityColors ? 'ON' : 'OFF'}`);
        });
    }
});